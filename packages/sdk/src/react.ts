import { replace, type createBrowserRouter } from 'react-router-dom'
...
export const createSubReactApp = ({ name, router }: { name: string; router: Router }) => {
  ...
  router.navigate = ((
    ...
    options?: Parameters<typeof router.navigate>[1]
  ) => {
    ...
    } as Parameters<typeof router.navigate>[1])
  }) as typeof router.navigate

  // 订阅路由状态变化
  const unsubscribe = router.subscribe(
    (state: {
      location: { pathname: string; search: string; hash: string; state: { replace: boolean } }
      ...
      window.top!.postMessage(
        {
          ...
          payload: {
            name,
            path: currentPath,
            replace: !!state.location.state.replace
          },
        },
        '*'
      )
